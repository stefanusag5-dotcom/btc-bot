...
async def ask_gemini(data: dict) -> str:
    """Имя функции сохранено для совместимости, внутри Groq"""
    if not groq_client:
    ...
    tf = data['tf']
    tf_context = {
        "15m": "Скальпинг/интрадей. Сделка живёт часы.",
        "1h":  "Интрадей. Сделка живёт 1-2 дня.",
        "4h":  "Свинг-трейдинг. Сделка живёт несколько дней.",
        "1d":  "Позиционная торговля. Сделка живёт недели.",
    }.get(tf, "Интрадей.")
    ...
    system = f"""{data['mode_personality']}
{tf_context}
СТРОГИЕ ПРАВИЛА:
- Отвечай ТОЛЬКО на основе цифр переданных тебе
- ЗАПРЕЩЕНО упоминать новости, геополитику, макроэкономику, ФРС, войны, выборы
- ЗАПРЕЩЕНО использовать знания о рыночных событиях из обучения
- Если фундаментал противоречит технике — игнорируй фундаментал
- ТП и СЛ уже рассчитаны — не пересчитывай их"""
...
Тренд {tf}: {data['trend_local']} | HTF: {data['trend_higher']}{conflict_line}
...
Ответ СТРОГО в формате (без повторения данных):
✅ ВЫВОД: войти / пропустить / ждать
💪 СИЛА: слабый / средний / сильный
⚠️ РИСК: [технический риск одним предложением]
💡 СОВЕТ: [одно конкретное действие с учётом таймфрейма {tf}]"""
    ...
    for model in GROQ_MODELS:
        for attempt in range(2):
            try:
                ...
                err = str(ex)
                logger.error(f"Groq model={model} attempt={attempt+1}: {err}")
                is_rate    = "429" in err or "rate" in err.lower() or "quota" in err.lower()
                is_unavail = "404" in err or "not found" in err.lower() or "decommissioned" in err.lower()
                if is_unavail:
    ...
    return "⏳ Groq: лимит запросов, попробуй позже"
...
async def handle_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        gemini_text = await ask_gemini(result) if groq_client else "AI отключён (нет GROQ_API_KEY)"
