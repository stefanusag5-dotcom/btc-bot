...
async def analyze_symbol(symbol, tf="15m", mode_cfg=None):
    ...
    weekly_task = asyncio.create_task(fetch_weekly_trend(symbol))
    (df, source, fr, oi), (df_htf, htf_label), df_daily, btc_dom, news, weekly_trend = await asyncio.gather(
        main_task, higher_task, daily_task, dom_task, news_task, weekly_task)
...
async def ask_ai(data: dict) -> str:
    ...
    system = """Ты опытный трейдер-аналитик. Твоя задача — дать СЦЕНАРНЫЙ анализ.
Не повторяй данные. Думай как трейдер у экрана: что ты видишь, что делаешь.
Отвечай по-русски, коротко и конкретно. Только цифры из данных."""

    user = f"""Монета: {data['symbol']} | ТФ: {tf} ({tf_ctx})
Цена закрытия свечи: {price} | Текущая: {data.get('current_price', price)}
Сигнал бота: {signal} | Скоринг: {score}/100
Режим рынка: {data.get('regime', {}).get('label', '')}
Неделя: {data.get('weekly_trend', '')}
RSI={data['rsi']} | ATR={data['atr_pct']}% | Дельта={data['delta']}
Тренд {tf}: {data['trend_local']} | HTF: {data['trend_higher']}
{'КОНФЛИКТ: ' + conflict if conflict else ''}
Дивергенция: {data.get('rsi_divergence') or 'нет'} | Пробой: {data.get('hvn_breakout') or 'нет'}
POC={poc} | HVN↑={hvn_a} | HVN↓={hvn_b}
Поддержки={sup} | Сопротивления={res}
Фандинг={data.get('funding_rate','N/A')}% | {data.get('oi_trend','')}
{'СЛ=' + str(sl_tp.get('sl','?')) + ' ТП1=' + str(sl_tp.get('tp1','?')) + ' ТП2=' + str(sl_tp.get('tp2','?')) + ' R/R=1:' + str(sl_tp.get('rr_ratio','?')) if sl_tp else ''}

Дай ответ СТРОГО в этом формате — не добавляй ничего лишнего:

🎯 СЦЕНАРИЙ 1 (основной): [опиши что должно произойти для входа и по какой цене]
🎯 СЦЕНАРИЙ 2 (альтернатива): [противоположный сценарий — когда НЕ входить]
📍 ЛУЧШИЙ ВХОД: [конкретная цена или уровень для лимитного ордера, почему именно там]
⚠️ ГЛАВНАЯ УГРОЗА: [одно предложение — что может сломать сделку]
✅ ИТОГ: войти по рынку / лимитный ордер на [цена] / пропустить"""
...
def calculate_limit_orders(result: dict) -> dict:
    """
    Когда нет чёткого сигнала (WATCH/НЕТ) или скоринг < 60,
    предлагает лимитные ордера:
    - От поддержки/HVN (ждём отскок)
    - На пробой уровня (импульс)
    """
    price   = result['price']
    ...
    if levels_below:
        ...
        orders.append({
            ...
            "tp1":   tp_limit,
            "dist":  dist_pct,
            "reason": f"HVN/поддержка на {buy_level} (-{dist_pct}% от цены)"
        })

    # Ордер 2: LONG на пробой сопротивления сверху
    if levels_above:
        break_level = levels_above[0]
        entry_break = round(break_level * 1.002, 6)  # чуть выше уровня
        ...
        orders.append({
            "type":  "📈 ЛИМИТ LONG (пробой сопротивления)",
            "entry": entry_break,
            "sl":    sl_break,
            "tp1":   tp_break,
            "dist":  dist_pct,
    ...
    # Ордер 3: SHORT от сопротивления/HVN сверху
    if levels_above:
        ...
        orders.append({
            ...
            "tp1":   tp_sell,
            "dist":  dist_pct,
            "reason": f"HVN/сопротивление на {sell_level} (+{dist_pct}% от цены)"
        })
    ...
    if (datetime.now().timestamp() - last_run) < 900:  # 15 минут
        ...
        for i in range(0, min(len(symbols), 80), 5):
            ...
            for result in results:
                ...
                # Лимитные ордера для сканера если сигнал слабоватый
                limit_data = None
                if result.get('score', 0) < 55 or result['signal'] == "⚠️ WATCH":
...
async def handle_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Telegram лимит 4096 символов — разбиваем на 2 сообщения если нужно
        TG_LIMIT = 4000
        if len(main_text) <= TG_LIMIT:
            await msg.edit_text(main_text, parse_mode='HTML')
        else:
            # Часть 1: технический анализ (до блока AI)
            ai_split = main_text.find("🧠 <b>AI:</b>")
            if ai_split > 0:
                part1 = main_text[:ai_split].rstrip()
                part2 = main_text[ai_split:]
                # Обрезаем каждую часть на всякий случай
                await msg.edit_text(part1[:TG_LIMIT], parse_mode='HTML')
                await update.message.reply_text(part2[:TG_LIMIT], parse_mode='HTML')
            else:
                # Просто обрезаем
