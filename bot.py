            ...
            async with session.get("https://fapi.binance.com/fapi/v1/ticker/24hr") as r:
                if r.status == 200:
                    ...
                    pairs = [
                        ...
                        and float(d.get('quoteVolume', 0)) >= 5_000_000   # фильтр $5M+
                    ]
...
TF_MAX_RISK = {"15m": 0.1, "1h": 0.14, "4h": 0.2, "1d": 0.27 is}
...
def calculate_sl_tp(signal, price, atr, hv_nodes, supports=None, resistances=None, tf="15m"):
    ...
    # СЛ строго в процентах — меняй SL_PCT в начале файла
    sl_pct = SL_PCT.get(tf, 0.05)

    if is_long:
        sl   = round(price * (1 - sl_pct), 6)
        risk = price - sl
        # ТП снэппятся к ближайшим уровням если близко
        ...
        tp3 = round(min(tp3_raw, price - risk*3.5, tp2 - price*0.01), 6)
...
def compute_score_and_signal(
    ...
    # Реальный максимум без экстра-сигналов ~75 (EMA+HTF+RSI+Delta+HVN)
    # С дивергенцией/пробоем до 106. Нормализуем к 75 как базе.
    max_possible = 75.0
...
def format_message(result, ai_text, is_scanner=False, limit_data=None):
    ...
    # Строим сообщение через список — надёжнее чем конкатенация
    parts = [
    ...
    parts += [
        ...
        f"Дельта: {e(result['delta'])}{fr_str}{oi_str}\n\n",
    ...
    if result.get('hack_news'):
        parts.append(f"💀 <b>Хаки:</b> {e(result['hack_news'][:100])}\n")
    parts += [
...
# ================== ПАМП/ДАМП ДЕТЕКТОР ==================
pump_scanner_active = {}   # chat_id -> True/False
...
def detect_pump_dump(df: pd.DataFrame, funding_rate: float = None) -> dict | None:
    """
    Памп/Дамп детектор с уровнями от ликвидности (HVN/POC).
    ПАМП: свеча >6.5% + объём >2x + RSI скакнул
    ДАМП: памп >10% за 5св + объём угасает + RSI >75
    """
    if len(df) < 30:
        return None
    ...
    # Volume Profile для уровней ликвидности
    ...
    # Уровень отката при пампе: ближайший HVN ниже или POC
    pullback_level = hvn_below_prices[0] if hvn_below_prices else poc_price
    pullback_pct   = round((price_now - pullback_level) / price_now * 100, 1)

    # Уровень входа в шорт при дампе: ближайший HVN выше
    short_level    = hvn_above_prices[0] if hvn_above_prices else round(price_now * 1.02, 6)
    short_sl       = round(float(last['high']) * 1.003, 6)  # выше максимума свечи
    short_tp1      = pullback_level  # цель — ближайший HVN ниже
    ...
    if candle_chg >= 6.5 and vol_ratio >= 2.0 and rsi_jump >= 10:
        ...
        long_sl  = round(pullback_level * 0.985, 6)
        long_tp1 = round(price_now * 1.05, 6)
        long_tp2 = round(price_now * 1.10, 6)
        return {
    ...
    # ══ ДАМП ══
    is_pump_zone  = total_pump >= 10.0
    is_rsi_hot    = rsi_now >= 75
    is_vol_fading = vol_fading
    is_funding_hi = (funding_rate or 0) >= 0.05
    dump_score    = sum([is_pump_zone, is_rsi_hot, is_vol_fading, is_funding_hi])
    ...
    if dump_score >= 3:
        ...
        return {
            ...
            "reason": (
                ...
                f"Объём {'↘️угасает' if is_vol_fading else '→держится'} | "
...
def format_pump_message(symbol: str, tf: str, det: dict) -> str:
    ...
    header = f"{icon} <b>АНОМАЛЬНЫЙ СИГНАЛ</b> | <b>{e(symbol)}</b> {tf}\n"
    stats  = (
        f"Цена: <b>{det['price']}</b> | {e(det['signal'])}\n"
        f"📊 {e(det['reason'])}\n"
    )
    ...
    if is_pump:
        ...
        levels = (
            f"\n📐 <b>УРОВНИ ОТ ЛИКВИДНОСТИ:</b>\n"
            f"POC (магнит): <b>{det['poc']}</b>\n\n"
            f"<b>📥 ЛОНГ на откате:</b>\n"
            f"├ Вход: <b>{det['pullback_level']}</b> (-{det['pullback_pct']}% от цены)\n"
            f"├ СЛ: {det['long_sl']} (ниже уровня)\n"
            f"├ ТП1: {det['long_tp1']} (+5%)\n"
            f"└ ТП2: {det['long_tp2']} (+10%)\n\n"
            f"<b>📤 ШОРТ от сопротивления:</b>\n"
            f"├ Вход: <b>{det['short_level']}</b> (HVN выше)\n"
            f"└ СЛ: {det['short_sl']} (выше хая свечи)\n\n"
            f"Дельта: {emoji} {det['buy_pct']}% покупок | "
            f"Суммарно: +{det['total_pump']}%"
        )
    else:
        levels = (
            f"\n📐 <b>УРОВНИ ОТ ЛИКВИДНОСТИ:</b>\n"
            f"POC (магнит): <b>{det['poc']}</b>\n\n"
            f"<b>📤 ШОРТ:</b>\n"
            f"├ Вход: <b>{det['short_level']}</b> (HVN — зона сопротивления)\n"
...
async def run_pump_scanner(app):
    """Сканер памп/дамп — запускается каждые 3 минуты"""
    ...
    try:
        ...
        for i in range(0, len(symbols), 10):
            ...
            # Грузим только основные данные — быстро
            tasks = [fetch_ohlcv(s, "15m") for s in batch]
            ...
            for j, raw in enumerate(raw_results):
                ...
                # Антиспам: одна монета не чаще раза в 30 минут
        ...
        for tf_scan in scan_tfs:
            ...
            for i in range(0, len(symbols), 5):
                ...
                for result in results:
                    ...
                    if (now - state.get(sig_key, 0)) < 7200:
                        continue
                    state[sig_key] = now
                    save_scanner_state(state)
                    ...
                    for chat_id in active_chats:
                        try:
                            ...
                            if result.get('sl_tp') and result['signal'] in ("🟩 LONG", "🟥 SHORT"):
                                trade_key = open_trade(symbol, tf_scan, result, chat_id, mode="scanner")
...
async def cmd_pump(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Управление памп/дамп детектором"""
    ...
    if cmd == "on":
        ...
        await update.message.reply_text(
            ...
            "Сигналы НЕ входят в статистику",
            parse_mode='HTML'
        )
    elif cmd == "off":
...
async def cmd_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cmd == "on":
        ...
        # Скан топ-100 монет на 15m и 1h — показываем все сигналы без порога
        msg = await update.message.reply_text(
            "🔍 Сканирую топ-100 монет (15m + 1h)\nПодожди ~2 минуты..."
        )
        ...
        for i in range(0, len(symbols), 5):
            ...
            for r in batch_results:
                ...
                elif sc < scanner_threshold and not (conflict and sc >= scanner_threshold + 15):
        ...
        scores_all = [r['score'] for r in (passed + blocked_score + blocked_conflict + no_signal)
                      if isinstance(r, str) and '(' in r
                      for sc in [r.split('(')[-1].rstrip(')')] if sc.isdigit()] or [0]
        # Считаем из уже собранных данных
        all_scored = passed + blocked_score + blocked_conflict
        scores_all = []
        for item in all_scored:
            try:
                sc = int(item.split('(')[-1].rstrip(')'))
...
async def handle_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if result['signal'] in ("🟩 LONG", "🟥 SHORT") and sl_tp:
            # HARD режим — только показываем, не мониторим и не в статистику
            if is_hard:
                await update.message.reply_text(
...
async def post_init(app):
    # Создаём таблицу в PostgreSQL при старте
    _ensure_pg_table()
    logger.info(f"Stats backend: {'PostgreSQL' if DATABASE_URL else 'JSON only'}")
...
# ================== СТАТИСТИКА (JSON + PostgreSQL) ==================
STATS_FILE   = Path("bot_stats.json")
DATABASE_URL = os.getenv("DATABASE_URL", "")

def _get_pg_conn():
    """Подключение к PostgreSQL если DATABASE_URL задан"""
    if not DATABASE_URL:
        return None
    try:
        import psycopg2
...
def _ensure_pg_table():
    """Создаём таблицу trades если не существует"""
    conn = _get_pg_conn()
    if not conn: return
    try:
...
def load_stats() -> dict:
    """Загружаем из PostgreSQL если есть, иначе из JSON"""
    conn = _get_pg_conn()
    if conn:
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT symbol,tf,signal,entry,exit_price,pnl_pct,exit_reason,score,won,mode,created_at::text FROM bot_trades ORDER BY id")
                rows = cur.fetchall()
            conn.close()
            trades = []
            for r in rows:
...
def save_stats(stats: dict):
    """Сохраняем в JSON (PostgreSQL пишем напрямую в record_trade_result)"""
    STATS_FILE.write_text(json.dumps(stats, indent=2, ensure_ascii=False))

def save_trade_to_pg(trade: dict):
    """Сохраняем одну сделку в PostgreSQL"""
    conn = _get_pg_conn()
    if not conn: return
    try:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO bot_trades (symbol,tf,signal,entry,exit_price,pnl_pct,exit_reason,score,won,mode)
...
def record_trade_result(symbol: str, tf: str, signal: str, entry: float,
    ...
    Hard режим и pump/dump не идут в основную статистику."""
    # Hard не пишем совсем — слишком шумный
    if mode == "hard":
    ...
    # Сохраняем в PostgreSQL (переживает редеплой)
    save_trade_to_pg(trade)
    # И в JSON как резервная копия
...
async def cmd_dbstats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        text = (
            ...
            f"<i>С {since} | Всего: {total} сделок</i>\n\n"
