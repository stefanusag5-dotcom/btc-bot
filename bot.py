        ...
        for tf_scan in scan_tfs:
            ...
            for i in range(0, len(symbols), 5):
                ...
                for result in results:
                    ...
                    # ── Фильтр 5: ranging_gate из скоринга ───────────────────
                    # Скоринг уже заблокировал ranging без дивергенции/пробоя
                    detail_f  = result.get('score_detail', {})
                    rang_gate = detail_f.get('ranging_gate', True)
                    if not rang_gate:
                        continue
...
async def run_backtest(symbol: str, tf: str, days: int) -> dict:
    ...
    # Look-forward: максимальное время жизни сделки в свечах
    # 15m: 48 свечей = 12 часов  (раньше было 16 = 4 часа — слишком мало для BTC)
    # 1h:  24 свечи  = 24 часа
    # 4h:  14 свечей = 56 часов
    # 1d:  7 свечей  = 7 дней
