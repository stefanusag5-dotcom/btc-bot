...
🎯 СЦЕНАРИЙ 1 (вход): [конкретное условие входа — какой уровень должен держаться или пробиться, цена входа]
⛔ ИНВАЛИДАЦИЯ: [конкретный уровень цены при котором сделка отменяется — не общие слова, а число из данных выше]
📍 ЛУЧШИЙ ВХОД: [конкретная цена лимитного ордера из уровней выше и почему именно там]
⚠️ УГРОЗА: [одна конкретная техническая причина почему может не сработать]
✅ ИТОГ: войти по рынку / лимит на [цена] / пропустить"""
...
async def check_trades(app):
    ...
    if MEXC_API_KEY:
        await sync_mexc_trades(app)  # передаём app для уведомлений
    trades = load_trades()
