...
def detect_pump_dump(df: pd.DataFrame, funding_rate: float = None) -> dict | None:
    """
    Определяет аномальное движение за последнюю закрытую свечу.
    
    ПАМП: свеча выросла >6.5% + объём >2x среднего + RSI вырос
    ДАМП-сигнал: суммарный памп >10% за 5 свечей + объём угасает + RSI >75
    
    Возвращает dict с типом сигнала или None.
    """
    if len(df) < 30:
        return None

    # Работаем с закрытыми свечами
    closed = df.iloc[:-1]
    last   = closed.iloc[-1]
    prev   = closed.iloc[-2]
    ...
    candle_chg = (price_now - price_prev) / price_prev * 100  # % изменение последней свечи
    ...
    rsi_jump   = rsi_now - rsi_prev5  # насколько RSI вырос за 5 свечей

    # Суммарный памп за последние 5 свечей
    price_5ago  = float(closed.iloc[-6]['close']) if len(closed) > 6 else price_prev
    total_pump  = (price_now - price_5ago) / price_5ago * 100

    # Объём на последних 2 свечах (угасание)
    ...
    if (candle_chg >= 6.5
        ...
        return {
            ...
            "reason": (
                f"Свеча +{candle_chg:.1f}% | "
                f"Объём x{vol_ratio:.1f} | "
                f"RSI {rsi_now:.0f} (+{rsi_jump:.0f} за 5св)"
            ),
            "action": (
                "📍 Вход на откате к ближайшей поддержке\n⚠️ Не гонись за ценой — жди коррекции 30-50%"
                if candle_chg < 10 else
                "📤 Рассмотри ШОРТ на перекупленности\n⚠️ Памп сильный — вход только от ключевого уровня"
            ),
        }

    # ══════════════════════════════
    # ДАМП-сигнал: памп угасает
    # ══════════════════════════════
    is_pump_zone  = total_pump >= 10.0          # суммарно уже +10%
    is_rsi_hot    = rsi_now >= 75               # RSI перегрет
    is_vol_fading = vol_fading                  # объём падает
    is_funding_hi = (funding_rate or 0) >= 0.05 # шортистов уже выдавили

    dump_score = sum([is_pump_zone, is_rsi_hot, is_vol_fading, is_funding_hi])

    if dump_score >= 3:  # минимум 3 из 4 условий
        confidence = "🔴🔴🔴" if dump_score == 4 else "🔴🔴"
        return {
            "type":       "DUMP",
            "signal":     f"🔻 ДАМП вероятен {confidence}",
            "price":      round(price_now, 6),
            "candle_chg": round(candle_chg, 2),
            "vol_ratio":  round(vol_ratio, 1),
            "rsi":        round(rsi_now, 1),
            ...
            "reason": (
                f"Памп +{total_pump:.1f}% за 5 св | "
                f"RSI {rsi_now:.0f} | "
                f"Объём {'угасает' if is_vol_fading else 'держится'} | "
                f"Фандинг {funding_rate or 0:.3f}%"
            ),
            "action": "📤 ШОРТ от текущей зоны\n🛑 СЛ: выше максимума памп-свечи\n🎯 ТП: -5% / -10% / возврат к началу памп",
        }
...
def format_pump_message(symbol: str, tf: str, det: dict) -> str:
    """Форматирует уведомление о памп/дамп"""
    e = html_escape
    is_pump = det['type'] == 'PUMP'

    icon = "⚡" if is_pump else "🔻"
    header = f"{icon} <b>АНОМАЛЬНЫЙ СИГНАЛ</b>\n"
    body = (
        f"<b>{e(symbol)}</b> {tf} | Цена: <b>{det['price']}</b>\n"
        f"{e(det['signal'])}\n\n"
        f"📊 {e(det['reason'])}\n\n"
        f"{e(det['action'])}\n\n"
        f"⚠️ <i>Не входит в статистику бота</i>"
    )
    if is_pump:
        emoji = "🟢" if det['buy_pct'] > 60 else "⚪"
        body += f"\n\nДельта покупок: {emoji} {det['buy_pct']}%\nСуммарный рост за 5 св: +{det['total_pump']}%"
    return header + body
                    ...
                    for chat_id in active_chats:
                        try:
                            ...
                            if result.get('sl_tp') and result['signal'] in ("🟩 LONG", "🟥 SHORT"):
                                trade_key = open_trade(symbol, tf_scan, result, chat_id)
...
async def cmd_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cmd == "on":
        ...
        for i in range(0, len(symbols), 5):
            batch = symbols[i:i+5]
            br = await asyncio.gather(*[analyze_symbol(s,"15m",mode_cfg) for s in batch], return_exceptions=True)
            scores_all += [r['score'] for r in br if r and not isinstance(r, Exception) and r.get('score')]
