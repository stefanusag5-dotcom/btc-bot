...
async def _fetch_klines_with_end(ticker, interval_bn, interval_bb, limit, session, end_ts: int):
    """Fetch klines ending at specific timestamp (ms). Falls back to regular fetch."""
    try:
        async with session.get("https://fapi.binance.com/fapi/v1/klines",
                params={"symbol": ticker, "interval": interval_bn,
                        "limit": limit, "endTime": end_ts}) as r:
            if r.status == 200:
                data = await r.json()
                if data:
                    return _parse_binance(data), "Binance Futures"
    except Exception as e:
        logger.warning(f"BF endTime {ticker}: {e}")
    try:
        async with session.get("https://api.binance.com/api/v3/klines",
                params={"symbol": ticker, "interval": interval_bn,
                        "limit": limit, "endTime": end_ts}) as r:
        ...
        logger.warning(f"BS endTime {ticker}: {e}")
    return None, None
...
async def fetch_btc_macro_regime() -> dict:
    """
    Определяет глобальный режим рынка по BTC на дневном и недельном TF.
    Кеш 30 минут — не меняется часто, не нужно пересчитывать каждый скан.

    Возвращает:
        regime:    'bull' | 'bear' | 'neutral'
        strength:  'strong' | 'weak'   — насколько выражен тренд
        allowed:   ['LONG'] | ['SHORT'] | ['LONG', 'SHORT']
        label:     строка для отображения
        ema50_1d:  значение EMA50 дневного
        price_1d:  последняя цена дневного
    """
    cache_key = "btc_macro_regime"
    now = datetime.now().timestamp()
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            # Дневные свечи — основа
            async with session.get(
                "https://fapi.binance.com/fapi/v1/klines",
                params={"symbol": ticker, "interval": "1d", "limit": 120}
            ) as r:
                if r.status != 200:
                    raise Exception(f"HTTP {r.status}")
                daily = await r.json()

            # Недельные свечи — подтверждение
            async with session.get(
                "https://fapi.binance.com/fapi/v1/klines",
                params={"symbol": ticker, "interval": "1w", "limit": 26}
            ) as r:
                weekly_ok = r.status == 200
                weekly    = await r.json() if weekly_ok else []

        if not daily or len(daily) < 50:
            raise Exception("недостаточно дневных данных")

        closes_1d = [float(c[4]) for c in daily]
        price     = closes_1d[-1]
        s         = pd.Series(closes_1d)

        ema20_1d = float(s.ewm(span=20).mean().iloc[-1])
        ema50_1d = float(s.ewm(span=50).mean().iloc[-1])
        ema100_1d = float(s.ewm(span=100).mean().iloc[-1])

        # Наклон EMA50 — насколько сильно трендит
        ...
        if weekly and len(weekly) >= 10:
            wcloses  = [float(c[4]) for c in weekly]
            ws       = pd.Series(wcloses)
            wema10   = float(ws.ewm(span=10).mean().iloc[-1])
            wema20   = float(ws.ewm(span=20).mean().iloc[-1])
            wcur     = wcloses[-1]
            weekly_bull = wcur > wema10 > wema20
            weekly_bear = wcur < wema10 < wema20

        # Определяем режим
        # BULL: цена > EMA20 > EMA50, наклон положительный, неделя подтверждает
        # BEAR: цена < EMA20 < EMA50, наклон отрицательный, неделя подтверждает
        # NEUTRAL: всё остальное

        price_above_ema50 = price > ema50_1d
        price_above_ema20 = price > ema20_1d
        ema20_above_ema50 = ema20_1d > ema50_1d

        is_bull = (price_above_ema50 and price_above_ema20
                   and ema20_above_ema50 and slope_10d > 0)
        is_bear = (not price_above_ema50 and not price_above_ema20
                   and not ema20_above_ema50 and slope_10d < 0)

        # Сила тренда
        if is_bull:
            strong = slope_20d > 3 and weekly_bull
            regime = "bull"
        elif is_bear:
            strong = slope_20d < -3 and weekly_bear
            regime = "bear"
        else:
            strong = False
            regime = "neutral"
        ...
        if regime == "bull":
            if strength == "strong":
                allowed = ["LONG"]           # сильный бычий — только лонги
                label   = "🟢🟢 Сильный бычий рынок (BTC) — только LONG"
            else:
                allowed = ["LONG"]           # слабый бычий — тоже только лонги
                label   = "🟢 Бычий рынок (BTC) — приоритет LONG"
        elif regime == "bear":
            if strength == "strong":
                allowed = ["SHORT"]          # сильный медвежий — только шорты
                label   = "🔴🔴 Сильный медвежий рынок (BTC) — только SHORT"
            else:
                allowed = ["SHORT"]
                label   = "🔴 Медвежий рынок (BTC) — приоритет SHORT"
        else:
            allowed = ["LONG", "SHORT"]      # нейтральный — оба направления
            label   = "⚪ Нейтральный рынок (BTC) — оба направления"
        ...
        logger.warning(f"fetch_btc_macro_regime: {e}")
        # Fallback — нейтральный, не блокируем торговлю
        fallback = {
            "regime": "neutral", "strength": "weak",
            "allowed": ["LONG", "SHORT"],
            "label": "⚪ BTC режим неизвестен — оба направления",
            "ema50_1d": 0, "ema20_1d": 0, "price_1d": 0,
            "slope_10d": 0, "slope_20d": 0,
        }
        _cache[cache_key] = {'val': fallback, 'ts': now - 1500}  # кеш 5 минут при ошибке
        return fallback
...
# ATR множители по классу волатильности монеты
# SL = atr * sl_mult → автоматически адаптируется к монете и рынку
_VOL_PARAMS = {
    # (sl_mult, tp1_r, tp2_r, tp3_r, max_risk_pct)
    # low:  BTC/ETH — тихие, ATR мал → SL 2×ATR, цели стандартные
    "low":    (2.0, 1.5, 2.5, 4.0, 3.0),
    # normal: SOL/BNB/средние альты
    "normal": (1.8, 1.5, 2.5, 4.0, 5.0),
    # high:   мелкие альты/волатильные — ATR большой, не надо умножать много
    "high":   (1.5, 1.5, 2.5, 3.5, 8.0),
}

# Пороги ATR% для классификации (по TF)
_ATR_THRESHOLDS = {
    "15m": {"low": 0.4,  "high": 1.2},
    "1h":  {"low": 0.8,  "high": 2.5},
    "4h":  {"low": 1.5,  "high": 5.0},
    "1d":  {"low": 3.0,  "high": 9.0},
}

def _get_vol_class(atr_pct: float, tf: str) -> str:
    """low / normal / high по реальному ATR%"""
    t = _ATR_THRESHOLDS.get(tf, _ATR_THRESHOLDS["15m"])
    if atr_pct <= t["low"]:
        return "low"
    if atr_pct >= t["high"]:
        return "high"
...
def calculate_sl_tp(signal, price, atr, hv_nodes, supports=None,
                    resistances=None, tf="15m", atr_pct: float = None):
    """
    ATR-based SL/TP — адаптируется под реальную волатильность монеты.

    BTC 15m ATR=0.15% → SL=2.0×ATR=0.3%  → TP1=0.45%  (реалистично)
    SIREN ATR=4%      → SL=1.5×ATR=6%    → TP1=9%     (реалистично)

    TP привязываются к ближайшим HVN/S&R уровням (snap).
    """
    if signal not in ("🟩 LONG", "🟥 SHORT"):
        return {}

    # Вычисляем atr_pct если не передан
    if atr_pct is None:
        atr_pct = atr / price * 100 if price > 0 else 1.0

    vol_class = _get_vol_class(atr_pct, tf)
    sl_mult, tp1_r, tp2_r, tp3_r, max_risk = _VOL_PARAMS[vol_class]
    ...
    # SL = ATR × множитель, минимум 0.2% от цены
    raw_risk = max(atr * sl_mult, price * 0.002)

    if is_long:
        sl   = round(price - raw_risk, 6)
        risk = price - sl
        tp1  = _snap_to_level(price + risk * tp1_r, hvn_above_p + res)
        tp2  = round(max(
            _snap_to_level(price + risk * tp2_r, hvn_above_p + res, 1.0),
            tp1 + price * 0.003), 6)
        far  = [n for n in above if n["price"] > tp2]
        tp3  = round(max(
            max(far, key=lambda x: x["strength"])["price"] if far
            else price + risk * tp3_r,
            price + risk * (tp3_r - 0.5),
            tp2 + price * 0.005), 6)
    else:
        sl   = round(price + raw_risk, 6)
        risk = sl - price
        tp1  = _snap_to_level(price - risk * tp1_r, hvn_below_p + sup)
        tp2  = round(min(
            _snap_to_level(price - risk * tp2_r, hvn_below_p + sup, 1.0),
            tp1 - price * 0.003), 6)
        far  = [n for n in below if n["price"] < tp2]
        tp3  = round(min(
            min(far, key=lambda x: x["strength"])["price"] if far
            else price - risk * tp3_r,
            price - risk * (tp3_r - 0.5),
            tp2 - price * 0.005), 6)
        tp3  = round(max(tp3, price * 0.001), 6)
    ...
    warns = []
    if rr < 1.5:
        warns.append("⚠️ R/R ниже 1.5")
    if risk_pct > max_risk:
        warns.append(f"⚠️ Риск {risk_pct}% — уменьши позицию")
...
def compute_score_and_signal(
        ...
        all_hvn_above: list = None, all_hvn_below: list = None,
) -> tuple:
    """
    Переработанный скоринг v2. Три ключевых принципа:

    1. ТРЕНД — фильтр направления, не баллы.
       Аптренд разрешает только LONG, даунтренд — только SHORT.
       Нарушение тренда = жёсткий штраф.

    2. HVN — контекстно-зависимая интерпретация.
       HVN сверху в аптренде = ЦЕЛЬ для лонга (магнит вверх).
       HVN сверху в даунтренде = СОПРОТИВЛЕНИЕ для шорта.
       Раньше HVN сверху всегда = шорт → 80% сигналов SHORT.

    3. ДИВЕРГЕНЦИЯ — обязательное условие в ranging.
       В боковике без дивергенции или пробоя HVN = нет сигнала.
       Единственный фактор с доказанным edge в данных бэктестов.
    """
    rsi_long  = mode_cfg["rsi_long"]
    rsi_short = mode_cfg["rsi_short"]
    hvn_mult  = mode_cfg["hvn_mult"]
    is_hard   = mode_cfg.get("label", "") == "\U0001f534 HARD"
    ...
    all_hvn_above = all_hvn_above or []
    all_hvn_below = all_hvn_below or []
    ...
    # ══════════════════════════════════════════════════════════════════════
    # ШАГ 1: ОПРЕДЕЛЯЕМ РАЗРЕШЁННЫЕ НАПРАВЛЕНИЯ по тренду
    # Тренд — фильтр, не источник баллов
    # ══════════════════════════════════════════════════════════════════════
    both_up   = trend_l == "UPTREND"   and trend_h == "UPTREND"
    both_down = trend_l == "DOWNTREND" and trend_h == "DOWNTREND"
    mixed_trend = not both_up and not both_down

    # Разрешённые направления
    long_allowed  = both_up   or mixed_trend
    short_allowed = both_down or mixed_trend

    # В чётком тренде: контртрендовый сигнал получает штраф
    trend_penalty_long  = -15.0 if both_down else 0.0
    trend_penalty_short = -15.0 if both_up   else 0.0
    ...
    # ══════════════════════════════════════════════════════════════════════
    # ШАГ 2: HVN — контекстно-зависимая интерпретация
    # ══════════════════════════════════════════════════════════════════════
    ...
    if top_hvn:
        ...
        if strength_ratio > hvn_threshold:
            hv_pen  = 0.75 if is_hv_coin else 1.0
            hvn_pts = min(25, strength_ratio * 8) * hvn_w * hv_pen

            if top_hvn['is_above']:
                if both_up:
                    # Аптренд: HVN сверху = МАГНИТ → лонг к цели
                    long_score  += hvn_pts
                    long_reasons.append(f"HVN {top_hvn['price']} — цель лонга ({strength_ratio:.1f}x)")
                elif both_down:
                    # Даунтренд: HVN сверху = СОПРОТИВЛЕНИЕ → шорт
                    short_score += hvn_pts
                    short_reasons.append(f"HVN {top_hvn['price']} — сопротивление ({strength_ratio:.1f}x)")
                else:
                    # Боковик/смешанный: нейтрально, меньше баллов
                    short_score += hvn_pts * 0.5
                    short_reasons.append(f"HVN {top_hvn['price']} сверху (боковик, {strength_ratio:.1f}x)")
            else:
                if both_down:
                    # Даунтренд: HVN снизу = ЦЕЛЬ для шорта
                    short_score += hvn_pts
                    short_reasons.append(f"HVN {top_hvn['price']} — цель шорта ({strength_ratio:.1f}x)")
                elif both_up:
                    # Аптренд: HVN снизу = ПОДДЕРЖКА → лонг от уровня
                    long_score  += hvn_pts
                    long_reasons.append(f"HVN {top_hvn['price']} — поддержка ({strength_ratio:.1f}x)")
                else:
                    long_score += hvn_pts * 0.5
                    long_reasons.append(f"HVN {top_hvn['price']} снизу (боковик, {strength_ratio:.1f}x)")

    # ══════════════════════════════════════════════════════════════════════
    # ШАГ 3: RSI — экстремальные зоны
    # ══════════════════════════════════════════════════════════════════════
    ...
    if rsi < rsi_long_thr:
        ...
        # RSI перепроданность только усиливает лонг, не даёт сигнал в даунтренде
        if both_down:
            pts = pts * 0.3  # слабый контртренд
        long_score += pts
    ...
    # ══════════════════════════════════════════════════════════════════════
    # ШАГ 4: ДИВЕРГЕНЦИЯ — главный фактор в ranging, важный везде
    # Бэктесты: DOT RSI div WR 75%, SIREN 1h WR 50%, BNB WR 50%
    # ══════════════════════════════════════════════════════════════════════
    if rsi_div:
        if "Бычья" in rsi_div:
            # Бычья дивергенция сильнее в даунтренде (разворот) и ranging
            div_pts = 30 if market == "ranging" else 22
            div_pts = div_pts * (0.7 if is_hv_coin else 1.0)
            long_score  += div_pts
            long_reasons.append(f"Бычья дивергенция RSI" + (" [ключевой]" if market == "ranging" else ""))
        elif "Медвежья" in rsi_div:
            div_pts = 30 if market == "ranging" else 22
            div_pts = div_pts * (0.7 if is_hv_coin else 1.0)
            short_score += div_pts
            short_reasons.append(f"Медвежья дивергенция RSI" + (" [ключевой]" if market == "ranging" else ""))

    # ══════════════════════════════════════════════════════════════════════
    # ШАГ 5: ПРОБОЙ HVN — сильный сигнал, работает в обоих направлениях
    # ══════════════════════════════════════════════════════════════════════
    if hvn_break:
        brk_pts = 20 if is_hv_coin else 28
    ...
    # ══════════════════════════════════════════════════════════════════════
    # ШАГ 6: СВЕЧА + ДЕЛЬТА — подтверждающие факторы
    # ══════════════════════════════════════════════════════════════════════
    ...
    # ══════════════════════════════════════════════════════════════════════
    # ШАГ 7: ШТРАФ ТРЕНДА — применяем к контртрендовым сигналам
    # ══════════════════════════════════════════════════════════════════════
    long_score  += trend_penalty_long
    short_score += trend_penalty_short

    # ══════════════════════════════════════════════════════════════════════
    # ШАГ 8: RANGING ФИЛЬТР
    # В боковике без дивергенции или пробоя — нет сигнала
    # ══════════════════════════════════════════════════════════════════════
    ranging_gate_long  = True
    ranging_gate_short = True
    if market == "ranging":
        has_div   = bool(rsi_div)
        has_break = bool(hvn_break)
        if not has_div and not has_break:
            ranging_gate_long  = False
            ranging_gate_short = False

    # ══════════════════════════════════════════════════════════════════════
    # ФИНАЛЬНЫЙ РАСЧЁТ
    # max_possible пересчитан: тренд(15) + HVN(25) + div(30) + пробой(28)
    #                          + RSI(15) + свеча(8) + дельта(8) = 129
    # Хороший сигнал = 3 фактора ≈ 60-80 → нормализуем к 88
    # ══════════════════════════════════════════════════════════════════════
    max_possible     = 88.0
    ...
    min_diff    = int((10 if is_hard else 18) * hv_boost)
    min_score   = int((45 if is_hard else 60) * hv_boost)
    min_factors = 2

    long_ok  = (long_score  > short_score and diff >= min_diff
                and normalized_score >= min_score
                and long_factors  >= min_factors
                and long_allowed and ranging_gate_long)

    short_ok = (short_score > long_score  and -diff >= min_diff
                and normalized_score >= min_score
                and short_factors >= min_factors
                and short_allowed and ranging_gate_short)

    if long_ok:
        signal = "\U0001f7e9 LONG"
        reason = " + ".join(long_reasons[:3])
    elif short_ok:
        signal = "\U0001f7e5 SHORT"
        reason = " + ".join(short_reasons[:3])
    elif is_hard and normalized_score >= int(40 * hv_boost):
        if trend_l == "UPTREND":
            signal, reason = "\U0001f7e9 LONG",  f"HARD: аптренд (скор {normalized_score})"
        elif trend_l == "DOWNTREND":
            signal, reason = "\U0001f7e5 SHORT", f"HARD: даунтренд (скор {normalized_score})"
        else:
            signal = "\U0001f7e9 LONG" if buy_pct > 52 else "\U0001f7e5 SHORT"
            reason = f"HARD: дельта {buy_pct:.0f}%"
    else:
        blocks = []
        if not long_allowed and not short_allowed:
            blocks.append("контртренд заблокирован")
        if market == "ranging" and not (bool(rsi_div) or bool(hvn_break)):
            blocks.append("боковик без дивергенции/пробоя")
        signal = "НЕТ СИГНАЛА"
        reason = (
            f"Скоринг {normalized_score}/100"
            + (f" | {', '.join(blocks)}" if blocks else "")
            + (f" | \u26a1Волатильная" if is_hv_coin else "")
        )
    ...
    detail = {
        ...
        "coin_vol":      coin_vol,
        "trend_filter":  "up" if both_up else ("down" if both_down else "mixed"),
        "ranging_gate":  ranging_gate_long and ranging_gate_short,
    }
...
async def analyze_symbol(symbol, tf="15m", mode_cfg=None):
    ...
    signal, reason, score, score_detail = compute_score_and_signal(
        ...
        all_hvn_above=hvn_above, all_hvn_below=hvn_below,
    )
    ...
    sl_tp = calculate_sl_tp(signal, price, atr, all_nodes, supports, resistances, tf, atr_pct=atr_pct)
    ...
    return {
        ...
        "vol_class":        sl_tp.get("vol_class", "normal") if sl_tp else "normal",
...
def format_message(result, ai_text, is_scanner=False, limit_data=None):
    ...
    parts += [
        ...
        (lambda vc: f"RSI: {result['rsi']} | ATR: {result['atr']} ({result['atr_pct']}%) "
                      + {'low':'🐢 низк.волат','normal':'⚡ ср.волат','high':'🔥 высок.волат'}.get(vc,'?')
                      + "\n")(result.get('vol_class','normal')),
        ...
        f"{esc(result.get('btc_macro_label',''))}\n" if result.get('btc_macro_label') else "",
        ...
        try:
            ...
            if not closed and key in trades:
                ...
                if sl_reached:
                    ...
                    trades.pop(key, None)
                    closed = True
                    # Kill switch: SL → увеличиваем серию
                    streak = _update_kill_switch("sl")
                    if streak >= KILL_SWITCH_SL_STREAK:
                        ks_msg = (
                            "\u26d4 <b>KILL SWITCH</b>\n"
                            + f"{streak} SL \u043f\u043e\u0434\u0440\u044f\u0434 | \u041f\u0430\u0443\u0437\u0430 {KILL_SWITCH_PAUSE_HOURS}\u0447\n"
                        )
                        if chat_id:
                            try:
                                await app.bot.send_message(
                                    chat_id=chat_id, text=ks_msg, parse_mode='HTML'
                                )
                            except:
                                pass
...
MAX_SIGNALS_PER_RUN  = 10   # мягкий лимит на прогон
_scanner_running     = False

# ── Kill Switch ──────────────────────────────────────────────────────────────
KILL_SWITCH_SL_STREAK  = 5     # пауза после N подряд SL
KILL_SWITCH_PAUSE_HOURS = 4    # часов паузы

def _get_kill_switch_state() -> dict:
    state = load_scanner_state()
    return {
        "paused_until":    state.get("ks_paused_until", 0),
        "sl_streak":       state.get("ks_sl_streak", 0),
        "last_result":     state.get("ks_last_result", ""),
    }

def _update_kill_switch(result: str, app=None):
    """result: 'sl' | 'win' | 'be'"""
    state = load_scanner_state()
    streak = state.get("ks_sl_streak", 0)

    if result == "sl":
        streak += 1
        state["ks_sl_streak"]   = streak
        state["ks_last_result"] = "sl"
        if streak >= KILL_SWITCH_SL_STREAK:
            pause_until = datetime.now().timestamp() + KILL_SWITCH_PAUSE_HOURS * 3600
            state["ks_paused_until"] = pause_until
            logger.warning(f"KILL SWITCH: {streak} SL подряд → пауза {KILL_SWITCH_PAUSE_HOURS}ч")
    else:
        # Любой не-SL результат сбрасывает серию
        streak = 0
        state["ks_sl_streak"]   = 0
        state["ks_last_result"] = result

    save_scanner_state(state)
    return streak

def _is_kill_switch_active() -> tuple:
    """Возвращает (active: bool, minutes_left: int)"""
    state     = load_scanner_state()
    pause_until = state.get("ks_paused_until", 0)
    now       = datetime.now().timestamp()
    if pause_until > now:
        minutes_left = int((pause_until - now) / 60)
...
async def run_scanner(app):
    ...
    try:
        ...
        ks_active, ks_mins = _is_kill_switch_active()
        if ks_active:
            logger.info(f"Scanner: KILL SWITCH активен, осталось {ks_mins} мин")
            return
        ...
        # ── BTC Macro Regime — определяем ОДИН РАЗ для всего прогона ────
        btc_regime = await fetch_btc_macro_regime()
        logger.info(f"Scanner BTC regime: {btc_regime['label']}")

        # Уведомляем если режим сменился с прошлого прогона
        prev_regime = state.get("btc_regime_last", "neutral")
        if btc_regime['regime'] != prev_regime:
            state["btc_regime_last"] = btc_regime['regime']
            save_scanner_state(state)
            regime_msg = (
                "📊 <b>Смена режима BTC</b>\n"
                + btc_regime["label"] + "\n"
                + f"EMA20: {btc_regime['ema20_1d']} | EMA50: {btc_regime['ema50_1d']}\n"
                + f"Наклон EMA50 (10д): {btc_regime['slope_10d']:+.2f}%"
            )
            for chat_id in active_chats:
                try:
                    await app.bot.send_message(
                        chat_id=chat_id, text=regime_msg, parse_mode='HTML'
                    )
                except Exception as ex:
                    logger.error(f"regime notify {chat_id}: {ex}")
        ...
        for tf_scan in scan_tfs:
            ...
            for i in range(0, len(symbols), 5):
                ...
                for result in results:
                    ...
                    # ── Фильтр 2: BTC Macro Regime — мультипликатор порога ──
                    # Не запрет, а повышение планки для контртрендовых сигналов.
                    # Bull: шорт требует score+20 (возможен при дивергенции+пробое)
                    # Bear: лонг требует score+20
                    # Neutral: оба направления на обычном пороге
                    sig_direction  = "LONG" if "LONG" in result['signal'] else "SHORT"
                    btc_r          = btc_regime['regime']
                    is_countertrend = (
                        (btc_r == 'bull'    and sig_direction == 'SHORT') or
                        (btc_r == 'bear'    and sig_direction == 'LONG')
                    )
                    counter_threshold = scanner_threshold + (
                        25 if btc_regime['strength'] == 'strong' else 15
                    )
                    if is_countertrend and result['score'] < counter_threshold:
                        continue

                    # ── Фильтр 3: HTF конфликт ───────────────────────────────
                    if result.get('htf_conflict'):
                        continue

                    # ── Фильтр 3: минимум 3 фактора ─────────────────────────
                    # 2 фактора недостаточно по данным бэктестов.
                    # В bull/bear при совпадении тренда — хватит 2.
                    detail = result.get('score_detail', {})
                    winning_reasons = (
                        detail.get('long_reasons', []) if 'LONG' in result['signal']
                        else detail.get('short_reasons', [])
                    )
                    trend_aligned = not is_countertrend
                    min_factors   = 2 if trend_aligned else 3
                    if len(winning_reasons) < min_factors:
                        continue
                    ...
                    # ── Фильтр 5: ranging — требуем оба фактора ─────────────
                    # Одной дивергенции или одного пробоя недостаточно в боковике.
                    # Нужны оба: дивергенция + пробой HVN.
                    market_reg = result.get('regime', {}).get('regime', 'mixed')
                    if market_reg == 'ranging':
                        has_div   = bool(result.get('rsi_divergence'))
                        has_break = bool(result.get('hvn_breakout'))
                        if not (has_div and has_break):
                            continue

                    # ── Фильтр 6: в чётком тренде требуем score +10 ──────────
                    tf_filter = detail_f.get('trend_filter', 'mixed')
                    if tf_filter != 'mixed' and result['score'] < scanner_threshold + 10:
                        continue
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "\n"
        "📈 Режим рынка:\n"
        "/regime — текущий BTC режим и что разрешено\n"
        "\n"
        "📊 Бэктест:\n"
...
async def cmd_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cmd == "on":
        ...
        # Kill switch status
        ks_active, ks_mins = _is_kill_switch_active()
        ks_streak = state.get("ks_sl_streak", 0)
        ks_str = (
            f"\u26d4 KILL SWITCH активен — осталось {ks_mins} мин\n"
            if ks_active else
            f"\u2705 Kill Switch: {ks_streak}/{KILL_SWITCH_SL_STREAK} SL подряд\n"
        )

        # BTC macro regime
        btc_r = _cache.get("btc_macro_regime", {}).get("val", {})
        btc_label = btc_r.get("label", "не определён")

        await update.message.reply_text(
            f"\U0001f4e1 Сканер: {'\U0001f7e2 активен' if active else '\U0001f534 выключен'}\n"
            f"Последний скан: {last_str}\n"
            f"Порог скоринга: {scanner_threshold}/100\n"
            f"Антиспам: {blocked} монет\n"
            + ks_str +
            f"BTC режим: {btc_label}\n\n"
            f"Команды:\n"
            f"/scan on — включить\n"
...
# ================== БЭКТЕСТ ==================
async def run_backtest(symbol: str, tf: str, days: int,
                       date_from: str = None, date_to: str = None) -> dict:
    """
    Прогоняет скоринг по историческим свечам.

    date_from / date_to — опциональные фильтры периода (YYYY-MM-DD).
    Если указаны — берём достаточно свечей чтобы покрыть период + индикаторы.
    Если нет — последние N дней как раньше.
    """
    ticker = symbol.replace("/", "")
    ...
    if dt_from and dt_to:
        # Считаем сколько дней в периоде + буфер для индикаторов
        period_days = (dt_to - dt_from).days + 1
        fetch_days  = period_days + 5   # +5 дней буфер для индикаторов
        fetch_limit = min(fetch_days * cpd + 150, 1500)
    else:
        n_candles   = days * cpd
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        # Если нужен конкретный период — запрашиваем с endTime
        if dt_to:
            end_ts = int(dt_to.timestamp() * 1000) + 86400000  # +1 день
            df_raw, source = await _fetch_klines_with_end(
                ticker, cfg[0], cfg[1], fetch_limit, session, end_ts
            )
        else:
    ...
    if dt_from or dt_to:
        ...
        if dt_from:
            mask = mask & (df_raw['timestamp'] >= pd.Timestamp(dt_from))
        if dt_to:
            # +1 день чтобы включить последний день полностью
            mask = mask & (df_raw['timestamp'] <= pd.Timestamp(dt_to) + pd.Timedelta(days=1))
        # Для индикаторов нужны свечи ДО периода — берём с запасом 60 свечей
        first_valid = df_raw[mask].index[0] if mask.any() else 0
        start_buf   = max(0, df_raw.index.get_loc(first_valid) - 60)
        df_raw      = df_raw.iloc[start_buf:].copy()
        # Пересчитываем маску после среза
        if dt_from:
            period_start_idx = (df_raw['timestamp'] >= pd.Timestamp(dt_from)).idxmax()
        else:
            period_start_idx = df_raw.index[60]
        if dt_to:
            period_end_mask = df_raw['timestamp'] <= pd.Timestamp(dt_to) + pd.Timedelta(days=1)
            period_end_idx  = df_raw[period_end_mask].index[-1] if period_end_mask.any() else df_raw.index[-1]
        else:
            period_end_idx = df_raw.index[-1]
    else:
    ...
    look_fwd = {"15m": 48, "1h": 24, "4h": 14, "1d": 7}.get(tf, 48)
    mode_cfg = TRADE_MODES["mid"]
    trades   = []

    # Стартуем с индекса 60 (буфер для индикаторов)
    # Если есть фильтр по дате — стартуем с нужного индекса
    if period_start_idx is not None:
        start_idx = max(60, df_raw.index.get_loc(period_start_idx)
                        if period_start_idx in df_raw.index else 60)
    else:
        start_idx = 60

    if period_end_idx is not None:
        end_idx = min(
            df_raw.index.get_loc(period_end_idx)
            if period_end_idx in df_raw.index else len(df_raw) - look_fwd - 1,
            len(df_raw) - look_fwd - 1
        )
    else:
        end_idx = len(df_raw) - look_fwd - 1
    ...
    for i in range(start_idx, end_idx):
        ...
        # SL/TP
        sl_tp = calculate_sl_tp(signal, price, atr, local_nodes, supports, resis, tf, atr_pct=atr_pct)
        if not sl_tp:
            continue
        ...
        if result == "open":
            if tp2_hit:
                ...
                # Expired = цена не дошла ни до TP ни до SL за look_fwd свечей
                # Выходим по последней цене окна (реалистично)
                last_future_price = float(future.iloc[-1]["close"]) if len(future) > 0 else price
                result    = "expired"
                exit_price = last_future_price
        ...
        # expired сделки — честный результат по закрытию окна
        won = pnl > 0.1  # небольшой буфер на комиссию
...
async def cmd_backtest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        await update.message.reply_text(
            ...
            "Использование:\n"
            "<code>/backtest BTCUSDT 15m 30</code> — последние 30 дней\n"
            "<code>/backtest BTCUSDT 15m 2024-03-01</code> — 30 дней от даты\n"
            "<code>/backtest BTCUSDT 15m 2024-03-01 2024-03-31</code> — период\n\n"
            "<b>Режимы для теста:</b>\n"
            "Бычий: 2024-02-01 → 2024-03-15\n"
            "Крэш: 2022-11-01 → 2022-12-01\n"
            "Боковик: 2024-09-01 → 2024-10-15\n"
            "Альт-сезон: 2024-11-01 → 2024-12-31",
            parse_mode='HTML'
        )
        return
    ...
    # Форматы:
    # /backtest BTCUSDT 15m 30
    # /backtest BTCUSDT 15m 2024-03-01 2024-03-31
    # /backtest BTCUSDT 15m 2024-03-01  (30 дней от даты)
    ...
    if len(args) > 2:
        arg2 = args[2]
        # Проверяем — это дата или число дней
        if re.match(r"\d{4}-\d{2}-\d{2}", arg2):
            date_from = arg2
            days      = 30
            if len(args) > 3 and re.match(r"\d{4}-\d{2}-\d{2}", args[3]):
                date_to = args[3]
                ...
                # Только дата начала — берём 30 дней от неё
                from datetime import datetime as _dt, timedelta as _td
                d1    = _dt.strptime(date_from, "%Y-%m-%d")
                date_to = (d1 + _td(days=30)).strftime("%Y-%m-%d")
                days  = 30
        elif arg2.isdigit():
            days = min(int(arg2), 180)

    period_str = (f"{date_from} → {date_to}" if date_from
                  else f"последние {days} дней")

    msg = await update.message.reply_text(
        f"\u23f3 Бэктест {symbol} {tf} | {period_str}\n"
        f"Это займёт 20–90 секунд.",
        parse_mode='HTML'
    )

    try:
        bt = await run_backtest(symbol, tf, days, date_from=date_from, date_to=date_to)
    except Exception as e:
    ...
    text = (
        ...
        f"<i>Период: {period_str} | Источник: {bt.get('source','?')} | Сигналов: {total}</i>\n\n"
...
async def cmd_regime(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показывает текущий BTC macro режим и что разрешено торговать."""
    msg = await update.message.reply_text(
        "⏳ Определяю режим рынка...", parse_mode='HTML'
    )
    try:
        r = await fetch_btc_macro_regime()
        allowed_str = " | ".join(r['allowed'])
        arrow = "↑" if r['slope_10d'] > 0 else ("↓" if r['slope_10d'] < 0 else "→")
        text = (
            f"📊 <b>BTC MACRO РЕЖИМ</b>\n\n"
            f"{r['label']}\n\n"
            f"<b>Технические данные:</b>\n"
            f"Цена: <b>{r['price_1d']}</b>\n"
            f"EMA20 (1d): {r['ema20_1d']}\n"
            f"EMA50 (1d): {r['ema50_1d']}\n"
            f"Наклон EMA50 (10д): {arrow} {r['slope_10d']:+.2f}%\n"
            f"Наклон EMA50 (20д): {r['slope_20d']:+.2f}%\n\n"
            f"<b>Разрешены в сканере:</b> {allowed_str}\n"
            f"<b>Сила тренда:</b> {r['strength']}\n\n"
            f"<i>Обновляется каждые 30 минут</i>"
        )
        await msg.edit_text(text, parse_mode='HTML')
    except Exception as e:
        await msg.edit_text(
            f"❌ Ошибка: {html_escape(str(e))}", parse_mode='HTML'
        )
