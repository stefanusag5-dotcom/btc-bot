...
def compute_score_and_signal(
    ...
    # ── 2. Тренд: требуем совпадения TF + HTF (макс ~25 баллов) ───────────
    # Одиночный тренд без подтверждения HTF даёт меньше баллов
    ...
    elif trend_l == "UPTREND":
        # Одиночный тренд — меньше баллов, не тянет на сигнал один
        long_score += 10 * trend_mult
        long_reasons.append("Аптренд TF (HTF не подтверждает)")
    elif trend_l == "DOWNTREND":
        short_score += 10 * trend_mult
        short_reasons.append("Даунтренд TF (HTF не подтверждает)")

    # ── 3. RSI — только значимые зоны (макс ~18 баллов) ───────────────────
    # Для волатильных монет RSI гуляет сильнее → пороги жёстче
    rsi_long_thr  = max(rsi_long  - (5 if is_hv_coin else 0), 20)
    rsi_short_thr = min(rsi_short + (5 if is_hv_coin else 0), 80)

    if rsi < rsi_long_thr:
        pts = 18 if rsi < 25 else (12 if rsi < 30 else 7)
        long_score += pts
        long_reasons.append(f"RSI {rsi} — перепроданность")
    elif rsi > rsi_short_thr:
        pts = 18 if rsi > 77 else (12 if rsi > 72 else 7)
        short_score += pts
    ...
    # - обычный: минимум 2 фактора, разрыв ≥ 18, скор ≥ 55
    # - hard:    минимум 1 фактор, разрыв ≥ 8,  скор ≥ 40
    # - волатильная монета: все пороги +15% жёстче
    hv_boost   = 1.15 if is_hv_coin else 1.0
    min_diff   = int((8  if is_hard else 18) * hv_boost)
    min_score  = int((40 if is_hard else 55) * hv_boost)
    min_factors = 1 if is_hard else 2
        ...
        try:
            ...
            if not t['tp1_hit']:
                ...
                if (is_long and check_tp >= t['tp2']) or (not is_long and check_tp <= t['tp2']):
                    ...
                    record_trade_result(
                        ...
                        t['sl'], t['tp1'], t['tp2'], t['tp3'], t['tp2'],
                        "tp2", 0, mode=t.get('mode', 'mid')
                    )
                    msgs.append(
                        f"🎯🎯 <b>ТП2 достигнут!</b>\n"
                        f"{t['symbol']} {t['tf']} | Цена: {round(price, 6)}\n"
                ...
                if (is_long and check_tp >= t['tp3']) or (not is_long and check_tp <= t['tp3']):
                    record_trade_result(
                        t['symbol'], t['tf'], t['signal'], t['entry'],
                        t['sl'], t['tp1'], t['tp2'], t['tp3'], t['tp3'],
                        "tp3", 0, mode=t.get('mode', 'mid')
                    )
                    msgs.append(
            ...
            if key in trades and not sl_just_moved:
                if (is_long and check_sl <= t['sl']) or (not is_long and check_sl >= t['sl']):
                    ...
                    record_trade_result(
                        t['symbol'], t['tf'], t['signal'], t['entry'],
                        t['sl'], t['tp1'], t['tp2'], t['tp3'], exit_p,
                        tag, 0, mode=t.get('mode', 'mid')
                    )
